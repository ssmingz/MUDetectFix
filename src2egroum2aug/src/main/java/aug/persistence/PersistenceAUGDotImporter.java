...
import static aug.persistence.PersistenceAUGDotExporter.*;
...
public class PersistenceAUGDotImporter extends DOTImporter<Node, Edge> {
    ...
    private static Edge createEdge(Node from, Node to, String label, Map<String, String> attributes) {
        ...
        if (edgeType == RepetitionTrueEdge.class) { // added by jaz
            return new RepetitionTrueEdge(from, to);
        }
        if (edgeType == SelectionTrueEdge.class) { // added by jaz
            return new SelectionTrueEdge(from, to);
        }
        if (edgeType == RepetitionFalseEdge.class) { // added by jaz
            return new RepetitionFalseEdge(from, to);
        }
        if (edgeType == SelectionFalseEdge.class) { // added by jaz
            return new SelectionFalseEdge(from, to);
        }
