...
public abstract class MuDetectStrategy implements DetectionStrategy {
    ...
    public DetectorOutput detectViolations(DetectorArgs args, DetectorOutput.Builder output) throws Exception {
        ...
        // added by jaz
        // for each violation, create a new fixed APIUsageExample according to pattern
        List<CompilationUnit> fixedTargets = createFix(violations).findFixes();
    ...
    // added by jaz
    private AUGFix createFix(List<Violation> violations) {
        return new AUGFix(violations);
    }
}
